...
def getUserInsights( params ) :
	...
	endpointParams = dict() # parameter to send to the endpoint
	endpointParams['metric'] = 'impressions,profile_views,reach,website_clicks,phone_call_clicks,follower_count,text_message_clicks,email_contacts' # fields to get back
...
def getUserInsights2( params ) :
	...
	endpointParams = dict() # parameter to send to the endpoint
	endpointParams['metric'] = 'audience_city,audience_country,audience_gender_age,audience_locale' # fields to get back
...
print("\n\tTotal Likes:") # when it was posted
print("\t" + str(response['json_data']['data'][0]['like_count']))
print("\n\tTotal comments:") # when it was posted
print("\t" + str(response['json_data']['data'][0]['comments_count'])) # when it was posted
...
print("\n---- DAILY USER ACCOUNT INSIGHTS (Day) -----\n") # section header
...
response = getUserInsights2( params ) # get insights for a user
print("\n---- DAILY USER ACCOUNT INSIGHTS (LifeTime) -----\n") # section header
